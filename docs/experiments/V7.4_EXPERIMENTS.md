...
## Additional Experiments (did not beat 60.98%)

### Experiment 5: Stacking with Meta-Learner
- OOF predictions from LR, XGB, LGBM used as features for meta-learner
- LogReg meta-learner: 59.59% (WORSE)
- Simple average: 60.49% (no improvement)
- OOF-weighted: 60.49% (no improvement)
- **Result**: Stacking underperformed simple blending

### Experiment 6: Feature Selection
- Tested top 30/50/75/100/150/222 features by LogReg coefficient
- Top 75: 60.41% blend
- Top 150: 60.89% blend
- **Result**: All configurations underperformed full feature set

### Experiment 7: CatBoost
- Tested multiple regularization configs
- Best: Conservative + 70% LR = 60.89%
- **Result**: Did not beat XGBoost ensemble

### Experiment 8: Triple/Quad Blends
- LR + XGB + LGBM: 60.89% (best at 75/10/15 split)
- LR + XGB + CatBoost: 60.81%
- Quad blend (LR+XGB+LGBM+Cat): 60.98% (ties best, doesn't improve)
- **Result**: No improvement over simple 80/20 LR+XGB

---

## Final V7.4 Model

**Best Configuration**: 80% LogReg + 20% XGBoost

| Metric | V7.3 | V7.4 | Change |
|--------|------|------|--------|
| **Accuracy** | 60.49% | **60.98%** | **+0.49pp** |
| **Log Loss** | 0.6702 | **0.6645** | -0.0057 |
| **ROC-AUC** | 0.6402 | **0.6425** | +0.0023 |
| **Brier** | ~0.237 | **0.2357** | improved |
| **A+ Bucket** | ~71.5% | **72.4%** | +0.9pp |

**Gap Analysis**:
- Target: 62%
- Achieved: 60.98%
- Remaining gap: **1.02pp**
- Progress: Closed 33% of gap from V7.3 to target
...
7. **Stacking doesn't help** - Models too correlated for meta-learner to add value
8. **Feature selection hurts** - All features contribute; removing any hurts performance
9. **CatBoost underperforms XGBoost** - XGBoost's regularization better suited to this data
10. **Diminishing returns** - After 80/20 LR+XGB, adding more models doesn't help
