#!/usr/bin/env python3
"""
V7.5: Train model with Feature Interactions

Adds 12 interaction terms between top predictors and contextual features:
- rolling_goal_diff_10 × divisional/rest/b2b
- rolling_high_danger_shots × divisional/rest/b2b
- season_goal_diff_avg × divisional/rest/b2b
...
from nhl_prediction.interaction_features import add_interaction_features
...
def main():
    print("=" * 80)
    print("V7.5: Training with Feature Interactions")
    print("=" * 80)
    ...
    # Add situational features (for context)
    print("=" * 80)
    print("[2/4] Adding situational features for context...")
    print("=" * 80)
    games_with_situational = add_situational_features(dataset.games)
    print("✓ Situational features added")
    print()

    # Add V7.5 interaction features
    print("=" * 80)
    print("[3/4] Adding V7.5 feature interactions...")
    print("=" * 80)
    features_with_interactions = add_interaction_features(dataset.features, games_with_situational)

    interaction_features = [c for c in features_with_interactions.columns if '_x_' in c]
    print(f"✓ Interaction features added: {len(interaction_features)}")
    print()

    print(f"Feature Summary:")
    print(f"  V7.0 base: {len(dataset.features.columns)}")
    print(f"  V7.5 interactions: {len(interaction_features)}")
    print(f"  Total: {len(features_with_interactions.columns)}")
    print()

    # Train/test split
    print("=" * 80)
    print("[4/4] Training V7.5 model...")
    print("=" * 80)
    train_mask = dataset.games['seasonId'].isin(TRAIN_SEASONS)
    test_mask = dataset.games['seasonId'] == TEST_SEASON

    X_train = features_with_interactions[train_mask]
    y_train = dataset.target[train_mask]
    X_test = features_with_interactions[test_mask]
    ...
    print(f"Features: {len(features_with_interactions.columns)}")
    print()

    # Train model
    print(f"Training V7.5 model (C={OPTIMAL_C})...")
    ...
    # Evaluate
    print("=" * 80)
    print("Evaluating V7.5 performance...")
    print("=" * 80)
    ...
    print(f"  V7.5 (+ interactions): {test_acc*100:.2f}%")
    ...
    # Feature importance
    print("=" * 80)
    print("Interaction Feature Importance:")
    print("=" * 80)
    lr = model.named_steps['clf']
    feature_names = features_with_interactions.columns
    coefficients = lr.coef_[0]

    # Get interaction features
    interaction_coefs = [(f, coefficients[list(feature_names).index(f)]) for f in interaction_features]
    interaction_coefs.sort(key=lambda x: abs(x[1]), reverse=True)

    print(f"{'Feature':<50s} {'Coefficient':>12s}  {'Abs Importance':>15s}")
    print("-" * 80)
    for feat, coef in interaction_coefs:
        print(f"{feat:<50s} {coef:12.4f}  {abs(coef):15.4f}")

    print()
    print("=" * 80)
    print("V7.5 Training Complete!")
