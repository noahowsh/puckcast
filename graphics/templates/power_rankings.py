...
def draw_team_tile(
    ...
    draw.text((x + S(10), y + S(8)), str(rank), fill=hex_to_rgb(PuckcastColors.TEXT_TERTIARY), font=rank_font)
    ...
    draw.text((x + (tile_size - abbrev_w) // 2, y + tile_size - S(32)), abbrev, fill=hex_to_rgb(PuckcastColors.TEXT_PRIMARY), font=abbrev_font)
    ...
    if delta != 0:
        ...
        draw.text((x + tile_size - S(32), y + tile_size - S(24)), delta_text, fill=delta_color, font=delta_font)
...
def generate_power_rankings_image(rankings: List[Dict], week_of: str) -> Image.Image:
    ...
    # Grid layout: 8 cols x 4 rows - larger tiles to fill space
    cols, rows = 8, 4
    tile_size = S(128)
    gap = S(12)
