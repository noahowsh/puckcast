#!/usr/bin/env python3
"""
Elo Pipeline Test - Test Elo configurations using actual pipeline

This modifies the pipeline temporarily to test different Elo parameters.
"""
...
from nhl_prediction import pipeline as pipeline_module
...
def custom_add_elo_features(
    ...
    """Custom Elo function with configurable dynamic home advantage."""
...
def build_dataset_with_custom_elo(seasons, **elo_params):
    """Build dataset with custom Elo parameters."""
    ...
    # Use custom Elo
    games = custom_add_elo_features(games, **elo_params)

    # Build features (same as pipeline)
    ...
    feature_columns = [
        ...
        "season_win_pct_diff", "season_goal_diff_avg_diff",
        "season_xg_diff_avg_diff", "season_shot_margin_diff",
        "games_last_6d_home", "games_last_3d_home", "games_last_6d_away", "games_last_3d_away",
        "goalie_trend_score_diff",
        "momentum_win_pct_diff", "momentum_goal_diff_diff", "momentum_xg_diff",
        "shotsFor_roll_10_diff", "rolling_faceoff_5_diff",
    ]
    ...
    features = games[available].copy()
    target = games["home_win"]

    return games, features, target


def test_config(seasons, config_name, **elo_params):
    """Test a configuration."""
    games, features, target = build_dataset_with_custom_elo(seasons, **elo_params)
...
def main():
    print("=" * 90)
    print("ELO PIPELINE TEST - FULL FEATURE SET")
    print("=" * 90)
    ...
    configs = [
        # Baselines
        ("Fixed adv=35 (V8.1)", {"home_advantage": 35, "dynamic_home_advantage": False}),
        ("Fixed adv=30", {"home_advantage": 30, "dynamic_home_advantage": False}),
        ("Fixed adv=25", {"home_advantage": 25, "dynamic_home_advantage": False}),

        # Current V8.5 dynamic
        ("V8.5 Dynamic (w=50)", {"dynamic_home_advantage": True, "home_adv_window": 50, "home_adv_scale": 1000}),

        # Alternative dynamic configs
        ("Dynamic w=100 s=700", {"dynamic_home_advantage": True, "home_adv_window": 100, "home_adv_scale": 700}),
        ("Dynamic w=100 s=500", {"dynamic_home_advantage": True, "home_adv_window": 100, "home_adv_scale": 500}),

        # Conservative (bounded range)
        ("Conservative (20-50)", {"dynamic_home_advantage": True, "home_adv_window": 100, "home_adv_scale": 600, "home_adv_min": 20, "home_adv_max": 50}),

        # Slower adaptation
        ("Slow adapt (w=150)", {"dynamic_home_advantage": True, "home_adv_window": 150, "home_adv_scale": 700}),
    ]
    ...
    for name, params in configs:
        print(f"\n⏳ Testing: {name}...")
        results = test_config(seasons, name, **params)
        ...
        print(f"   4-season: {avg_4:.1%}, 2025-26: {acc_2526:.1%}")
    ...
    # Find best
    baseline = all_results["Fixed adv=35 (V8.1)"]
    ...
    for name, results in all_results.items():
        ...
        change_4 = avg_4 - baseline_4
        change_2526 = acc_2526 - baseline_2526

        # Score: don't hurt 4-season too much, improve 2025-26
        if change_4 >= -0.003:  # Allow up to 0.3pp drop in 4-season
            score = change_4 + change_2526
            if score > best_improvement:
                best_improvement = score
                best_config = (name, avg_4, acc_2526, change_4, change_2526)
...
- 4-season: {best_config[1]*100:.1f}% ({best_config[3]*100:+.2f}pp vs baseline)
- 2025-26: {best_config[2]*100:.1f}% ({best_config[4]*100:+.2f}pp vs baseline)
- Total improvement: {(best_config[3]+best_config[4])*100:+.2f}pp

This configuration should be used in the pipeline.
""")
    else:
        print("\nConsider keeping the baseline or using a more conservative approach.")
